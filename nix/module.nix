...
in
{
  options.grow = {
    ...
    grafana = {
      enable = (lib.mkEnableOption "the grow grafana monitoring.") // {
        default = true;
      };
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services = {
      grow-agent = lib.mkIf cfg.agent.enable {
        ...
        environment = {
          ...
          GROW_AGENT_CONFIG_PATH = pkgs.writeText "grow-agent-config.json" (builtins.toJSON cfg.agent.config);
        };
      };

      grow-server = lib.mkIf cfg.server.enable {
        wantedBy = [ "multi-user.target" ];
        serviceConfig = {
          Type = "exec";
